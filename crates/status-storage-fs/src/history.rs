...
use status_domain::history::*;
#[cfg(test)]
use status_domain::models::Status;
...
impl HistoryStore {
    pub fn open(cfg: HistoryConfig) -> Result<Self> {
        crate::create_directories(&cfg.directory.join("daily"))
    ...
    pub fn append(&self, snap: &Snapshot) -> Result<()> {
        crate::create_directories(&self.root)?;
        ...
        // A killed append may leave a truncated final line. Keep it separate
        // so the next complete observation survives recovery and compaction.
        ...
        let mut bytes = serde_json::to_vec(snap)?;
        bytes.push(b'\n');
            ...
            .map(|line| {
                line.map(|bytes| {
                    String::from_utf8(bytes)
                        .unwrap_or_else(|_| "<invalid UTF-8 history line>".to_owned())
                })
            })
...
mod tests {
    ...
    #[yare::parameterized(
        ascii = { b"{\"v\":1,\"t\":" as &[u8] },
        unicode = { b"{\"v\":1,\"servers\":{\"\xc3" as &[u8] }
    )]
    fn interrupted_append_preserves_the_next_complete_sample(tail: &[u8]) -> Result<()> {
        let dir = tempdir()?;
        ...
        fs::write(store.snapshots_path(), tail)?;
        store.append(&snapshot(1_700_000_000))?;
        store.compact_and_rotate(Utc.timestamp_opt(1_700_000_100, 0).unwrap())?;
        assert_eq!(store.read_valid_snapshots()?.len(), 1);
        Ok(())
    }
    #[test]
    fn interrupted_compaction_is_idempotent() -> Result<()> {
        let dir = tempdir()?;
        let store = HistoryStore::open(HistoryConfig {
            directory: dir.path().to_owned(),
        ...
        let old = snapshot(1_700_000_000);
        store.append(&old)?;
        // Simulate termination after the daily rollup was committed but before
        // replacing raw history. Recovery must not double-count that day.
        let date = Utc.timestamp_opt(old.t, 0).unwrap().date_naive();
        store.write_json_atomic(&store.daily_path(date), &roll_up(date, &[old], &[]))?;
        let now = Utc.timestamp_opt(1_700_000_000 + 3 * 86400, 0).unwrap();
        store.compact_and_rotate(now)?;
        store.compact_and_rotate(now)?;
        let view = store.load_for_window(now, 90)?;
        assert!(view.raw.is_empty());
        assert_eq!(view.daily.len(), 1);
        assert_eq!(view.daily[0].snapshots_count, 1);
        Ok(())
    }
